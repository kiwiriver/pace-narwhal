...
def generate_html_header(title, title2, logo_path=None, resolution_factor=1.0, quality=95):
    ...
    logo_css = [
        ...
        '  .header-content { position: relative; padding-right: 260px; margin-bottom: 20px; }',
    ]
    ...
    return '\n'.join([
        ...
        *logo_css,
        ...
        '  .criteria-btn.map { background-color: #3F51B5; color: white; }',
        ...
        '  .view-csv-btn { background: #17a2b8; color: white; border: none; padding: 6px 12px; border-radius: 3px; margin: 3px 5px 3px 0; cursor: pointer; font-size: 12px; }',
        '  .view-csv-btn:hover { background: #138496; }',
        ...
        # CSV Modal styling
        '  .csv-modal { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); z-index: 1000; }',
        '  .csv-modal-content { background: white; margin: 2% auto; border-radius: 8px; max-width: 95%; max-height: 90%; overflow: hidden; display: flex; flex-direction: column; }',
        '  .csv-modal-header { display: flex; justify-content: space-between; align-items: center; padding: 20px; border-bottom: 1px solid #eee; background: #f8f9fa; }',
        '  .csv-content-container { flex: 1; overflow: auto; padding: 20px; }',
        '  .csv-table { border-collapse: collapse; width: 100%; font-family: monospace; font-size: 11px; }',
        '  .csv-table th, .csv-table td { border: 1px solid #ddd; padding: 4px 8px; text-align: left; white-space: nowrap; }',
        '  .csv-table .header-row { background-color: #007bff; color: white; font-weight: bold; }',
        '  .csv-table .even-row { background-color: #f8f9fa; }',
        '  .csv-table .odd-row { background-color: white; }',
        '  .csv-file-item { margin: 8px 0; padding: 10px; background: #f8f9fa; border-radius: 4px; border-left: 4px solid #007bff; }',
        # Loading animation
...
def generate_javascript_and_footer():
    ...
    return '\n'.join([
        ...
        '  const mapSection = document.getElementById(varId + "_map");',
        '  ',
        ...
        '    })',
        '    .then(csvText => {',
        '      displayCSVModal(csvText, filename, url);',
        '    })',
        '    .catch(error => {',
        '      showErrorModal(error.message, filename, url);',
        '    });',
        '}',
        '',
        ...
        '        <div style="font-size: 18px; margin-bottom: 20px;">Loading CSV...</div>',
        ...
        '}',
        '',
        'function displayCSVModal(csvText, filename, downloadUrl) {',
        '  closeModal(); // Close loading modal',
        '  const modal = createModal();',
        '  const tableHtml = formatCSVAsTable(csvText);',
        '  ',
        '  modal.innerHTML = `',
        '    <div class="csv-modal-content">',
        ...
        '        <div>',
        '          <button onclick="downloadCSV(\'${downloadUrl}\', \'${filename}\')" ',
        '                  style="background: #28a745; color: white; border: none; padding: 8px 16px; ',
        '                         border-radius: 4px; margin-right: 10px; cursor: pointer; font-size: 14px;">',
        '            💾 Download',
        '          </button>',
        ...
        '}',
        '',
        'function showErrorModal(errorMessage, filename, downloadUrl) {',
        '  closeModal(); // Close loading modal',
        '  const modal = createModal();',
        '  ',
        '  modal.innerHTML = `',
        '    <div class="csv-modal-content">',
        '      <div class="csv-modal-header">',
        '        <h3 style="margin: 0; color: #dc3545;">❌ Error Loading ${filename}</h3>',
        '        <div>',
        '          <button onclick="downloadCSV(\'${downloadUrl}\', \'${filename}\')" ',
        '                  style="background: #28a745; color: white; border: none; padding: 8px 16px; ',
        '                         border-radius: 4px; margin-right: 10px; cursor: pointer; font-size: 14px;">',
        ...
        '        <p style="color: #666; font-size: 16px; margin-bottom: 15px;">${errorMessage}</p>',
        '        <p style="font-size: 14px; color: #888;">The file might not exist yet or access might be restricted.<br>You can try downloading it directly instead.</p>',
        '      </div>',
        '    </div>',
        '  `;',
        '  ',
        ...
        '}',
        '',
        'function formatCSVAsTable(csvText) {',
        '  const lines = csvText.trim().split("\\n");',
        '  const maxRows = 1000; // Show up to 1000 rows',
        '  const displayLines = lines.slice(0, maxRows);',
        '  ',
        '  let html = `<div style="margin-bottom: 15px; color: #666; font-size: 14px; padding: 10px; background: #f0f8ff; border-radius: 4px;">',
        '               📊 Showing <strong>${displayLines.length}</strong> of <strong>${lines.length}</strong> rows',
        '               ${lines.length > maxRows ? " (truncated for performance)" : ""}',
        '             </div>`;',
        '  ',
        '  html += `<div style="overflow: auto; max-height: 60vh; border: 1px solid #ddd; border-radius: 4px;">`;',
        ...
        '      const rowClass = index === 0 ? "header-row" : (index % 2 === 0 ? "even-row" : "odd-row");',
        '      ',
        '      html += `<tr class="${rowClass}">`;',
        '      cells.forEach(cell => {',
        '        const displayCell = cell.length > 50 ? cell.substring(0, 50) + "..." : cell;',
        '        html += `<${tag} title="${escapeHtml(cell)}">${escapeHtml(displayCell)}</${tag}>`;',
        '      });',
        '      html += `</tr>`;',
        '    }',
        '  });',
